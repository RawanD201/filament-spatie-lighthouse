...
### Fixed
- Fix View button silently showing nothing: `latestResults()` and `getHistory()` now use `orderByDesc('id')` and exclude `raw_results` from the query, preventing MySQL sort buffer OOM when loading the detail panel
